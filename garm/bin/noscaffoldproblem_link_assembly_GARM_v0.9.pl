...
open INTERLACE, ">linked_scaffolds.txt" || die "$0 : Can't write to linked_scaffolds.txt\n";
	...
	unless ($complete{$scaff}) {
		...
		foreach my $other (keys %matches) {
			...
			if ($count == $contigs) {
				...
				print INTERLACE "$scaff is merged completely in $other\n"; #<STDIN>;
			} else {
				print INTERLACE "$scaff is merged partially in $other\n" unless ($done{$scaff} or $done{$other}); #<STDIN>;
				$done{$scaff}++;
				$done{$other}++;
				#my $fasta1 = GETFASTA($scaff);
				#my $fasta2 = GETFASTA($other);
				#print "this is scaff $scaff"; <STDIN>;
				#PRINT(\@{$newscaffs{$scaff}});
				#print "this is scaff $other"; <STDIN>;
				#PRINT(\@{$newscaffs{$other}});
				#REOVLSCAFF($scaff, $other, $fasta1, $fasta2);
...
open ONE, ">reoverlap/$scaff1.fasta" || die "$0 : Can't write to reoverlap/$scaff1.fasta\n";
open TWO, ">reoverlap/$scaff2.fasta" || die "$0 : Can't write to reoverlap/$scaff2.fasta\n";
...
my $ide = $overlapide;
#print OVLLOG "$scaff1.vs.$scaff2\tgap=$gap"; #<STDIN>;
...
system ("cd reoverlap; $mumbin/nucmer --maxmatch --prefix=$scaff1.vs.$scaff2 $scaff1.fasta $scaff2.fasta; wait; $mumbin/show-coords -H -c -l -o -r -I $ide $scaff1.vs.$scaff2.delta | egrep 'BEGIN|END|CONTAIN|IDE'  | sort -k10,10nr > $scaff1.vs.$scaff2.coords; cd .."); 
...
foreach my $elem (@{$newscaffs{$scaff}}) {
	...
	my $seq = "";
	if ($orient) {
		$seq= GetData::revcomp($mergedseqs{$contig}) if $mergedseqs{$contig};
		$seq = GetData::revcomp($extras{$contig}) if $extras{$contig};
		$seq = GetData::revcomp($singletons{$contig}) if $singletons{$contig};
	} else {
		$seq = $mergedseqs{$contig} if $mergedseqs{$contig};
		$seq = $extras{$contig} if $extras{$contig};
		$seq = $singletons{$contig} if $singletons{$contig};
	}
	$seqs .= "$seq\n";
			...
			if ($bigindex == 0 and $smallindex == $smallcount-1) {
				...
				for (my $index = ($smallindex-$goback); $index <= $smallindex; $index++) {
					if ($bigindex < 0) {
						...
						#print "goback first second $first | $second"; <STDIN>;
			...
			if ($bigindex == $bigcount-1 and $smallindex == 0) {
				...
				for (my $index = ($smallindex+1); $index <= $smallcount-1; $index++) {
					if ($bigindex > $bigcount-1) { ## END OF BIG SCAFFOLD
						...
						my($first, $second) = INTERLACE(\@smallelem, \@bigelem);
						#print "add first second $first | $second"; <STDIN>;
