	...
	if ($scaffolds{$scaff}) {
		...
		if ($scaffsextras{$scaff} eq $howmany) {
			...
			foreach my $index (0..$howmany-1) {
				...
				print RECOV "$extras{$contig}"; #<STDIN>;
				delete $scaffsextras{$contig};
				delete $extras{$contig};
...
foreach my $name (sort keys %scaffcontigs) {
	...
	for (my $index = 0; $index < $scaffcount; $index++) {		
		...
		if ($index == $scaffcount-1) { ### END OF THE SCAFFOLD
			...
			if ($line1 and $line2) {
				...
				if ($mergedcontig1 eq $mergedcontig2) { ### CHECK ORIENT AND DISTANCE
					...
					#$mergedcount++; ### DECIDE TO USE OR NOT
...
#open NEWSCAFF, "rescaffold.fasta" || die "$0 : Can't write to rescaffold.fasta\n";
...
foreach my $old (sort { scalar @{$newscaffs{$b}} <=> scalar @{$newscaffs{$a}} } keys %newscaffs) {
	...
	if ($howmany == 1) {
		...
		my $seq = $mergedseqs{$contig} || die "contig $contig mergedseq $mergedseqs{$contig}\n";
		my $dir = (split /\s+/, $check[0])[3];
		$seq = GetData::revcomp($mergedseqs{$contig}) if $dir;
		print NEWAGPSEQ ">$contig\_no_gaps\n$seq\n";
		#delete $mergedseqs{$contig};
	} else {
		unless ($complete{$old}) {
...
my $smallsize = 0;
my $smalldir = 0;
if ($smallcount == 2) { ### just have to fit the other contig in the other scaffold
	if ($newscaffs{$small}[0] eq $link) { ## link is the first contig get second;
		$smallname = (split /\s+/, $newscaffs{$small}[1])[0];	
		$smallsize = (split /\s+/, $newscaffs{$small}[1])[1];
		$smalldir = (split /\s+/, $newscaffs{$small}[1])[3] if (split /\s+/, $newscaffs{$small}[1])[3];
		
		my $gaplink = (split /\s+/, $newscaffs{$small}[0])[2];
		my $lennext = (split /\s+/, $newscaffs{$small}[1])[1];
		$fit = $gaplink+$lennext;
	} else {
		$smallname = (split /\s+/, $newscaffs{$small}[0])[0];	
		$smallsize = (split /\s+/, $newscaffs{$small}[0])[1];
		$smalldir = (split /\s+/, $newscaffs{$small}[1])[3] if (split /\s+/, $newscaffs{$small}[1])[3];
		
		#print "--->$newscaffs{$small}[0]"; <STDIN>;
		#print "original $scaffolds{$small}[0]"; <STDIN>;
		my $gapprev = (split /\s+/, $newscaffs{$small}[0])[2];
		my $lenprev = (split /\s+/, $newscaffs{$small}[0])[1];
		#print "$gapprev+$lenprev"; <STDIN>;
		$fit = $gapprev+$lenprev;
		$goback = 1;
	}
	for (my $index = 0; $index <= $bigcount-1; $index++) {
		my $check = (split /\s+/, $newscaffs{$big}[$index])[0];
		my $othergap = 0;
		if ($check eq $link) {
			#print "$check index $index goback=$goback\n"; <STDIN>;
			if ($goback) {
				$othergap = (split /\s+/, $newscaffs{$big}[$index-1])[2] if (split /\s+/, $newscaffs{$big}[$index-1])[2];
				#print "--->$othergap-$fit"; <STDIN>;
				my $newgap = $othergap-$fit;
				#print "*$smallname *$smallsize *$newgap *$smalldir"; <STDIN>;
				my $newelem = "$smallname $smallsize $newgap $smalldir";
				push(@newarray, $newelem);
				push(@newarray, $newscaffs{$big}[$index]) unless ($index == $bigcount-1);;
			} else {
				$othergap = (split /\s+/, $newscaffs{$big}[$index])[2];
				my $newgap = $othergap-$fit;
				my $newelem = "$smallname $smallsize $newgap $smalldir";
				push(@newarray, $newscaffs{$big}[$index]) unless ($index == $bigcount-1);
				push(@newarray, $newelem);
			...
			push(@newarray, $newscaffs{$big}[$index]);
		}
	}
	$complete{$small} = 1;
	#print "resolved"; <STDIN>;
} else {
	#print "fuck..."; <STDIN>;
	my $goback = 0;
	my $add = 0;
	...
	for (my $bigindex = 0; $bigindex <= $bigcount-1; $bigindex++) {
		...
		if ($check eq $link) {
			...
			if ($goback and $add) {
				print "big pedo..."; <STDIN>;
			}
			
			
			elsif ($goback and $add == 0) {
			
			### FINISH THIS!!! ####
				print "remember you had to fix this?\n"; <STDIN>;
				
			} 
			
			### CHEKC THIS ROUTINE!!! AGAIN!!!!
			elsif ($add and $goback == 0) {
				#print "have to add $add elements"; <STDIN>;
				for (my $index = ($smallindex+1); $index <= $add; $index++) {
					#print "index $index $newscaffs{$small}[$index]"; <STDIN>;
					if ($newscaffs{$big}[$bigindex+1]) {
						#print $newscaffs{$big}[$bigindex]; <STDIN>;
						$othergap = (split /\s+/, $newscaffs{$big}[$bigindex])[2] if (split /\s+/, $newscaffs{$big}[$bigindex])[2];
						my $otherlen = (split /\s+/, $newscaffs{$big}[$bigindex])[1];
						my $otherdir = (split /\s+/, $newscaffs{$big}[$bigindex])[3];		
						$smallname = (split /\s+/, $newscaffs{$small}[$index])[0];	
						$smallsize = (split /\s+/, $newscaffs{$small}[$index])[1];
						$smalldir = (split /\s+/, $newscaffs{$small}[$index])[3] if (split /\s+/, $newscaffs{$small}[$index])[3];
			
						#my $gaplink = (split /\s+/, $newscaffs{$small}[$index-1])[2];
						my $gaplink = $othergap;
						#my $lennext = (split /\s+/, $newscaffs{$small}[$index])[1];
						my $lennext = $smallsize;
						#print "line 577 $gaplink+$lennext"; <STDIN>;
						$fit = $gaplink+$lennext;
					
						if ($othergap+$otherlen > $fit) { #contig from smallscaff goes inside the gap of bigscaff
							my $newgap = $othergap-$fit;
							#print "contig from smallscaff $small goes inside the gap of bigscaff $big"; <STDIN>;
							#print "$newgap = $othergap-$fit"; <STDIN>;
							#print ">$smallname >$smallsize >$newgap >$smalldir"; <STDIN>;
							my $newelem = "$smallname $smallsize $newgap $smalldir";
							
							#print "bigindex=$bigindex $newscaffs{$big}[$bigindex]"; <STDIN>;
							#print "small=$index $newelem"; <STDIN>;
							
							push(@newarray, $newscaffs{$big}[$bigindex]);
							push(@newarray, $newelem);
						} else { #contig from bigscaff goes inside the gap of smallscaff
							my $newgap = $fit-$othergap;
							#print "contig from bigscaff $big goes inside the gap of smallscaff $small"; <STDIN>;
							#print "$newgap = $fit-$othergap"; <STDIN>;
							my $newelem = (split /\s+/, $newscaffs{$big}[$bigindex])[0]." $otherlen $newgap $otherdir";
						...
						$bigindex++;
					} else { #reached the end of the bigscaffold!!!
						#print "reached the end of the bigscaffold"; <STDIN>;
						push(@newarray, $newscaffs{$small}[$index]);
						$stop = 1;
					}
				}
