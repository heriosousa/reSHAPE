...
#print $log; <STDIN>;
#my $check1 = scalar @qcgen;
#my $check2 = scalar @scffiles;
#print "In module - $check1 - $check2 - $log"; <STDIN>;
...
#print "   -r: fasta file with repeats (RECOMMENDED but optional)\n";
...
#print "   -e: repeat match length at the end of contigs [Default 200]\n";
#print "      (only if repeats file provided [-r])\n";
#print "   -s: If scaffolds provided, number of STDEV for gap distances (default 1)\n";
...
#$shortest = `~as9/bin/headers_tamano.pl $genome | sort -n -k 2 | head -n 1 | cut -f 2`;
...
if (-s "$scf" and -s "$singlet") {
	...
	#system ("cat $scf $singlet > genome$count\_split_all.fasta");
...
#print MERGE "$amospath/make-consensus_poly -B -e 0.03 -b $prefix.bnk && \ \n";
...
my $pwd = shift;
my $prefix = shift;
my $stdevfactor = shift;

#system("cp $pwd/$prefix/merging/extras.fasta $pwd/$prefix/scaffolding");
...
#print SCF "sh $pwd/$prefix/scaffolding/reoverlap/GARM.merge.sh && \ \n";
print SCF "cp $pwd/$prefix/scaffolding/reoverlap/reoverlap.merged.fasta $pwd/$prefix/scaffolding/reoverlap/reoverlap.edited.read.placed $pwd/$prefix/scaffolding && \ \n";
print SCF "$garmbin/fix_scaffs_with_reoverlap.pl reoverlap.merged.fasta reoverlap.edited.read.placed new_scaffolds.txt.fasta.agp new_scaffolds.txt.fasta.all_split.fasta && \ \n";
print SCF "$garmbin/scaff_split.pl new_scaffolds.txt.fasta.agp.fixed.fasta\n";


# INTRODUCE  fix_scaffs_with_reoverlap.pl

#print SCF "mv new_scaffolds.txt.fasta $prefix.final_scaffolds.fasta && \ \n";
#print SCF "$garmbin/scaff_split.pl $prefix.final_scaffolds.fasta\n";
...
# print RET "cat $pwd/$prefix/merging/extras.fasta $pwd/$prefix/merging/$prefix.singletons.fasta $pwd/$prefix/input/*.leftovers > $pwd/$prefix/final_bin.fasta\n";
